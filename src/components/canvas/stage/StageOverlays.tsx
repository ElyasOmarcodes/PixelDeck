...
export function StageChrome({
  ...
  return <>
    ...
    {editingGroupId && <div style={{ position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(124,110,246,0.95)', borderRadius: 8, padding: '6px 14px', zIndex: 100, pointerEvents: 'all', boxShadow: '0 4px 20px rgba(124,110,246,0.4)' }}>
      <span style={{ color: '#fff', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}><Icon name="group" size={13} />Editing group</span>
    ...
    <div style={{ position: 'absolute', bottom: 12, right: 12, display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(15,15,19,0.88)', backdropFilter: 'blur(8px)', borderRadius: 8, padding: '4px 8px', border: '1px solid rgba(255,255,255,0.1)', zIndex: 100, pointerEvents: 'all' }}>
      <button onClick={(e) => { e.stopPropagation(); setZoom(Math.max(0.05, zoom - 0.02)) }} title="Zoom out" style={{ background: 'none', border: 'none', color: '#e8e8f0', cursor: 'pointer', padding: '2px 4px', lineHeight: 1, display: 'flex', alignItems: 'center' }}><Icon name="minus" size={14} strokeWidth={2.2} /></button>
      ...
      <button onClick={(e) => { e.stopPropagation(); setZoom(Math.min(4, zoom + 0.02)) }} title="Zoom in" style={{ background: 'none', border: 'none', color: '#e8e8f0', cursor: 'pointer', padding: '2px 4px', lineHeight: 1, display: 'flex', alignItems: 'center' }}><Icon name="plus" size={14} strokeWidth={2.2} /></button>
